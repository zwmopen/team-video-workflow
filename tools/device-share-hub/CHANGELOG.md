...
## USB 一键安装脚本 - 2026-08-14

- 新增 `scripts/install-usb-apk.ps1`：自动寻找电脑端最新 APK、筛选已授权 USB Android 手机、读取已安装版本，只对确实需要升级的设备执行 `adb install -r`。
- 安装后重新读取包名版本和版本码；已是同版或更新版时跳过，多台设备需要升级时要求用户明确选择，避免误装和重复安装。
- 桌面提供 `相册_USB一键安装.cmd` 双击入口；桌面入口只负责启动项目中的唯一脚本源。
