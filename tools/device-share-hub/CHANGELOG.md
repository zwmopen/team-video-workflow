...
## Windows V4.3.30 - 2026-09-27 - 把「在线相册服务」从散落的 .cmd/.ps1 收进客户端设置（DSH-119）

> **一句话**：手机端「电脑在线相册」读的是一个 Python 服务（默认 45835 端口）。
> 在此之前，客户端源码里**连 45835 这个数字都没出现过** —— 服务没起来时手机只会一直转圈，
> 用户既不知道有这个服务存在，也不知道该去点哪个黑框。

### 现象

手机端在线相册连不上电脑时，界面就停在「正在连接电脑在线相册…」。想确认服务状态只能靠手动跑脚本：

| 想做的事 | 以前要去跑 |
|---|---|
| 看状态 | `scripts\online_gallery-status.cmd` 或 `check_online_gallery.ps1` |
| 启动 | `scripts\online_gallery.cmd` |
| 重启 | `restart_online_gallery.ps1 -Restart`（还得记得这个参数） |
| 开机自启 | `install-autostart.ps1` |

四个能力散在四个文件里，客户端自己一个都不知道。

### 根因

`windows-native/src/main.cpp` 里只出现过手机端端口 45833，**没有任何一行提到 45835**。
客户端把「内容分发」和「在线相册服务」当成了两个世界，后者被完全留在脚本层。

### 修复

新增 `windows-native/src/online_service.h` / `online_service.cpp`，在「设置」窗口新增
**「在线相册服务（手机看电脑相册靠它）」** 分组，一次性把上面四个脚本的能力收进来：

| 以前要去跑的 | 现在在客户端里点 |
|---|---|
| `online_gallery.cmd` | 「启动」 |
| `restart_online_gallery.ps1 -Restart` | 「重启」 |
| （以前只能去任务管理器杀进程） | 「停止」 |
| `online_gallery-status.cmd` / `check_online_gallery.ps1` | 状态三行，**每 5 秒自动刷新** |
| `install-autostart.ps1` | 「开机自动启动服务」勾选框 |
| （无） | 「浏览器打开」「刷新」，以及解释器 / 脚本路径的「更改…」 |

状态三行分别是：

1. **状态** —— 运行中 / 未运行 · 手机可见作品数 · 服务版本
2. **详情** —— 端口 · 代码是否陈旧 · 更新中转是否正常 · 文件实时监听 · 扫描跳过的异常目录数
3. **地址** —— `http://<电脑局域网 IP>:45835`，用**只读输入框**而不是纯文本，
   这样手机自动扫不到电脑时用户能选中复制走、手动填。

### 设计与取舍

| 点 | 为什么这么做 |
|---|---|
| 探测走**后台线程** + `PostMessage` 回 UI 线程 | 探测是一次本机 HTTP 请求；放 UI 线程上跑，服务假死时设置窗口会整块卡住 |
| 探测结果带**自增序号**，过期结果丢弃 | 慢的那一次回来得晚，会把新状态覆盖成旧的 |
| 序号只比较**低 32 位** | `WPARAM` 在 32 位构建上是 `unsigned int`，拿 64 位原值去比被截断的 `WPARAM` 会**永远不等** ⇒ 状态再也不刷新 |
| 探测**节流 1 秒** | `RefreshSettingsWindow()` 有十几个调用入口（选目录、改阈值、开窗口…），不节流每次都会真的发一次请求 |
| WinHTTP 用 `WINHTTP_ACCESS_TYPE_NO_PROXY` | 这台机器上有 Clash 系统代理，走代理探 127.0.0.1 会被绕到外网去 |
| 停止前**先验身份**再结束进程 | 只按端口找 PID 就 `TerminateProcess` 很危险：端口是用户能手改的，万一撞上别的程序就把人家的进程杀了 |
| 开机自启写的是 `pythonw + 脚本` | 要开机跑起来的是**服务**，不是客户端本体 |
| 工作目录设成脚本所在目录 | 脚本里有一堆相对路径依赖，从别处启动会找不到素材库 |
| 脚本位置**逐级向上**找 6 层 | 开发期 exe 在 `build\Release`、发布期在 `out`，写死层数必然有一种布局找不到 |

### 闸门（CI 上跑纯逻辑部分）

新增 `tests/online_service_tests.cpp`，覆盖 8 个纯逻辑函数：命令行拼装、脚本位置候选、
启动前检查、状态解析、三行文案。系统操作（启动 / 停止 / 探测 / 注册表）留在真机，不进 CI。

⚠️ **用自定义 `CHECK` 而不是 `assert`**：CI 编的是 Release（`NDEBUG`），`assert` 会被整段编译掉。
仓库里已有的两个 C++ 测试就是这么写的 —— **实际上在 Release 下是空跑的假闸门**。
新测试不吃这一套：失败会打印行号并返回非零退出码。

两个「取值定位」的陷阱用例（能被变异打掉才说明判据有效）：

- `scanThrottle` 在前且带一个假的 `count`，`scanErrors` 在后带真的 `count`：
  取值器不先定位到 `scanErrors`，就会把 `scanThrottle` 的 count 当成扫描错误数。
- `thumbnail.active = false` 在前，`watchdog.active = true` 在后：
  不先定位到 `watchdog`，就会把缩略图状态当成文件监听状态。

### 编译前自检

本机没有 MSVC，**CI 是唯一编译闸门**，所以先在源码层面把能查的都查掉：
新用到的 `online_service::X` 是否都声明过、头文件声明的函数 cpp 是否都实现了、
花括号/圆括号是否平衡、6 个窗口句柄的「声明 / 创建 / 清理」是否三处配对、
8 个 IDC 编号是否唯一且都被真正用上。

⚠️ 做这个检查时自己也踩了两个坑，记在这里免得下次再踩：
- 删注释的正则会把 `L"http://192.168.1.23"` 里的 `//` 当成注释起点，从那儿一路删到行尾，
  把字符串截成半截，括号统计就全乱了 ⇒ 括号统计必须喂**原始文本**。
- 用裸 `find("Probe")` 找声明位置会先命中 `struct ProbeResult`，
  用裸 `find("Restart")` 会先命中注释里的 `-Restart（重启）` ⇒ 必须按 `名字\s*\(` 定位。

### 版本号

Windows 客户端 V4.3.29 → **V4.3.30**。
...
## DSH-114 — 扫描缓存**永远命中不了**：TTL 5 秒 < 单次扫描 5.2~10 秒

### 症状

手机每次连接/刷新都要在「正在连接电脑在线相册…」上**干等 8~11 秒**。
实测同一个接口间隔 7 秒连打三次：11.42 / 10.63 / 7.95 秒 —— **没有一次是快的**。

### 根因（一个非常隐蔽的算术错误）

`WorkScanner.scan()` 的缓存判据写死 `now - self._last_scan_time < 5.0`，
但**一次全量扫描本身就要 5.2~10 秒**（473 个作品：逐个读 `文案.txt` +
枚举图片 + 跑槽位守卫正则）。

> **扫描耗时 > 缓存时长 ⇒ 缓存刚写完就已经过期 ⇒ 缓存命中率恒为 0**

最直白的证据：

| | 耗时 |
|---|---|
| 改前：`scan()` 紧接着再 `scan()` | **5.244 秒**（本该是 0） |
| 改后：`scan()` 紧接着再 `scan()` | **0.0000 秒** |

也就是说，那个「5 秒缓存」从第一天起就没起过作用，而且因为不报错，
没人知道它坏了 —— 又是一个**静默故障**。

### 修法

TTL `5.0` → `SCAN_CACHE_TTL = 30.0`。

新鲜度**不靠时间，靠 DSH-110 的 watchdog**：它每 60 秒轮询一次，
一旦发现增删改就 `scan(force=True)` 主动作废缓存。所以 TTL 放宽到 30 秒
**不会**让手机看到更旧的数据 —— 之前 5 秒 TTL 也没有比 watchdog 更快。

顺带把服务 `version` 从 `1.0.0` 提到 `1.1.0`（行为对外可辨识）。

### 效果

同样间隔 7 秒打三次：`0.025s / 0.015s / 7.85s`
（第三次是被 watchdog 或手机端动作作废缓存后重扫，属正常）。
改前是 `11.42 / 10.63 / 7.95`。**三次里两次瞬时，改前零次。**

### 闸门 A17（41 项）

3 条判据：常量存在 / scan 用它 / 不许再出现写死的 `5.0`。
自检两项均正确 FAIL（把判据改回写死 5.0 → `ttlUsed=False`；把常量改回 5.0 → `ttlConst=False`）。

**回归**：服务端 29 项单元测试全过（放宽 TTL 没打破任何「改完立刻重扫」的用例）。



## DSH-115 — 中转出网代理不再写死 7897（Clash 端口会跳、节点会抽风）

### 症状

DSH-113 建的「局域网更新中转」忽然代取不到安装包：
`/download/apk` 只返回一个 **110 字节**的报错体
（`{"ok": false, "error": "<urlopen error [WinError 10054] ...>"}`）。
手机拿到它连体积校验都过不了，等于中转静默失效。

### 根因

`UPDATE_UPSTREAM_PROXY` 写死 `http://127.0.0.1:7897`。而：

- **Clash 混合端口会在 7890 / 7897 之间回跳** —— 2026-09-25 CDP 内容产线就因
  此整条停产过一次（守护环境变量停在 7890、实际端口跳到 7897）；
- 代理节点本身也会偶发掉线（本次实测 `WinError 10054 远程主机强迫关闭了一个现有的连接`，
  同一时刻 7890 正常、7897 抽风，几分钟后又自己恢复）。

写死任何一个端口，都会在它跳走/抽风时让中转**静默失效** —— 不报错，就是不给你包。

### 修法

出网改为**多候选轮试**：

1. 上次成功的代理（记在 `_UPDATE_PROXY_HINT`，典型情况下零额外开销）
2. 环境变量 `DSH_UPDATE_PROXY` 显式指定
3. `7897 → 7890 → 7891 → 7892`
4. 直连兜底（本机直连实测 10 秒超时，留着只为环境变化）

新增 `_update_fetch(req, timeout)`：第一个成功的赢并把地址记成下次提示；
清单和安装包**两处出网都走它**。`/api/online/status` 的 `updateRelay.proxy`
改为暴露**当前真正生效**的那个（原来是死的常量）。

### 闸门 A18（42 项）

6 条判据，源码级回退自检三项全部正确 FAIL：
- 把安装包出网回退成单点 opener → `noSingle=False`
- 删掉 7890 候选 → `twoPorts=False`
- 删掉候选常量 → `candidates=False`

⚠️ 自检踩到的坑：服务脚本是**纯 CRLF**，第一版变异串只写 `\n` 匹配不上，
脚本直接 assert 失败 —— 变异串必须带 `\r\n`。

**回归**：服务端 29 项单测全过。



## DSH-116 — 更新中转的安装包缓存只按版本落名（同版本重发 ⇒ 手机校验必失败）

**发现**: 2026-09-26（全屋巡检「看看具体状态」时抓到）
**影响端**: 服务端 `online_gallery_service.py`（局域网更新中转）

### 现象

中转看起来一切正常：`/api/online/status` 里 `downloads` 在涨、
`downloadFailures=0`、`lastError` 为空；手机也确实能下到 883308 字节的 APK。
但**装不上** —— Android `UpdatePackageValidator` 算出的 sha256 与清单里声明的
对不上，直接判「更新包校验失败」并把包删掉。
...
实测 v0.8.61 就被重发过一次：字节不同，体积却一模一样（都是 883308）。

于是缓存命中旧字节，配的却是新清单里的 sha256：

- 清单声明 `362908b0…`（GitHub 的 SHA256SUMS.txt 也是这个）
- 中转端出去 `45635c6a…`（上一次构建留下的旧字节）
- 旧的命中分支 `if isfile(...)` 读完**直接 return，完全不校验**

只有「缓存不存在、重新下载」那条路才校验 sha —— 而这条路永远走不到。

### 修法

1. `_update_cache_file(kind, version, sha)`：缓存名把 sha256 前 12 位编进去；
2. 命中分支也重算一遍 sha，不一致就 `os.remove` 后重新下载；
3. 不一致次数进 `_UPDATE_RELAY_STATS["shaMismatch"]`，当前端出去的包进
   `cachedSha` —— 两者都在 `/api/online/status` 的 `updateRelay` 里一眼可见。

### 闸门 A19（契约总数 43）

8 条判据，源码级回退 4 处全部正确 FAIL：

- 缓存名去掉 sha → `shaInName=False`
- 命中分支不校验 → `hitVerify=False`
- 不一致时不删旧文件 → `discard=False`
- 健康快照字段改名 → `health=False`

⚠️ 自检踩到的坑：变异串**不能是空串** —— 空串在文件里出现十几万次，
反向替换的锚点直接失效，还会把文件**留在变异态**。一律用「同长度改名」做变异。

**回归**：契约 43/43；服务端单测 29 项全过；实取 `/download/apk`
883308 字节，sha256 `362908b0…` 与清单、与 GitHub SHA256SUMS.txt 三方一致。

## 文档 — 局域网更新中转说明（2026-09-26）

- 新增 **`docs/LAN_UPDATE_RELAY.md`**：把 DSH-113~116 一次讲透 —— 为什么必须有它
  （电脑有代理、手机没有）、链路图、三个对外接口、清单里**改了什么 / 没改什么**
  （sha256 一个字节都不能动，否则手机必判废）、四条硬规则、DSH-116 详解与三套字节证据、
  健康自查（一条 curl + 三方 sha256 比对）、排障手册（症状→原因→确认→修）、验收清单。
- `README.md` 抬头版本号从 **0.8.44 / 0.8.25** 更正为 **0.8.61 / 0.8.44**，
...
## Windows V4.3.32 - 2026-09-27 - 电脑客户端升级：设置窗口能滚能缩放 + 在线手机面板 + 桌面快捷方式（DSH-122 / DSH-123 / DSH-125）

**影响端**：仅 Windows 电脑端（`windows-native/`），手机端与服务端无改动

### DSH-125 — 设置窗口底部点不到（先修，因为它挡住了后面要加的东西）

| 项 | 内容 |
|---|---|
| 现象 | 设置窗口写死 760×900，**且不可缩放**；内容底部在 y=864 |
| 后果 | 1366×768 / 1440×900 笔记本上，窗口比屏幕工作区还高 ⇒ 底部「常规与软件」整组和「关闭」按钮掉到屏幕外，拉不回来，也没有滚动条 |
| 为什么之前没暴露 | 开发机都是 1080p 以上的大屏，900px 刚好放得下 |

改动：

- 初始尺寸按 `SystemParametersInfoW(SPI_GETWORKAREA)` 自适应：`height = clamp(工作区高 - 60, 480, 900)`。
  拿不到工作区时退回 1280×800，绝不能算出 280px 宽的窄条。
- 窗口样式加 `WS_THICKFRAME | WS_MAXIMIZEBOX | WS_VSCROLL` ⇒ 可以缩放、可以最大化、能滚动。
- 内容高度**常量化**（`kSettingsContentBottom`），滚动时按「初始 y − 偏移量」整体平移所有子控件。
- `WM_GETMINMAXINFO` 设最小 620×420，防止被拉成什么都看不见的小条。
- 控件初始 y 在 `WM_CREATE` **全部建完之后**才记录 —— 早一步会漏掉后面建的控件，那些控件滚不动。

### DSH-122 — 「在线手机」面板：服务端接口早就有了，客户端没入口

服务端本来就有 `GET /api/online/phones`（扫 /24 找在线手机）和
`POST /api/online/sync-phone-counts`（把手机本地分享次数回写到电脑），
但**客户端源码里这两个字符串一次都没出现过** —— 想看「哪台手机在线」只能自己去跑
`scripts/phone_sync.py`。

新增 `src/phone_panel.{h,cpp}`，设置窗口加一组「在线手机（看谁在看你的电脑相册）」：

| 元素 | 说明 |
|---|---|
| 状态行 | 在线几台 · 扫描时间；有非本产品占端口时会另说一句；一台都没有时说明前提（同网段 + App 在前台） |
| 设备列表 | 一台一行：`IP:端口  设备名（机型） V版本 · 手机内 N 个作品`；未验证的会带上具体原因 |
| **刷新** | 丢掉服务端 30 秒缓存，重扫整个 /24（要等几秒） |
| **预演同步** | 干跑：只报告会回写哪些，**不落盘** |
| **同步次数** | 真落盘，把手机端分享次数补回电脑端元数据 |

边界（照抄服务端，客户端不另起一套口径）：**只补次数不搬文件**、**只增不减**。

### DSH-123 — 桌面快捷方式一键生成

`scripts/create-desktop-shortcuts.py` 原本要手动跑、还依赖 pywin32。
新增 `src/desktop_shortcut.{h,cpp}`，直接用 `IShellLink` COM 生成，设置窗口
「常规与软件」里加了「桌面快捷方式」按钮。

- 三个入口：启动 / 状态 / 开机自启，图标各不相同（图标一样桌面分不清）。
- 「启动」用最小化窗口：那是后台服务，弹黑框会被当成崩溃顺手关掉。
- 图标名与描述不含 emoji，并有 `HasSurrogatePair()` 守卫兜底。
- 目标 .cmd 不在时会明确标注「⚠ 双击会失败」—— 这种 .lnk 建得出来但点了没反应，
  属于典型静默失效。
...
| 文件 | 覆盖 |
|---|---|
| `tests/phone_panel_tests.cpp` | 真实面板解析、6 条陷阱、拒绝非面板数据、文案口径、同步结果、请求体转义 |
| `tests/desktop_shortcut_tests.cpp` | spec 拼装、目录带尾斜杠、emoji 守卫、空 spec 早退 |

6 条陷阱分别是：①嵌套 `devices` ②值里不平衡右括号 ③值里转义引号
④设备对象内嵌子对象同名字段 ⑤多台设备字段串台 ⑥光有 `{"ok":true}` 不算面板数据。
...
| 层 | 手段 | 结果 |
|---|---|---|
| 判据有区分度 | `_verify_phone_panel.py`：Python 逐行复刻解析核心，用开关翻转模拟「守卫被去掉」 | 6 条陷阱**逐个关掉对应守卫 ⇒ 6/6 全部 FAIL**，还原后回绿 |
| C++ 里确实有这些判据 | `scripts/check-cpp-test-assertions.py`（P1–P7） | 全绿，7 个测试目标全部已注册 |
| 真编译真运行 | CI 的 `ctest` | 见 CI |

### 意外收获：复刻器抓出一个会让全部解析失效的真 BUG

`ObjectBody()` 里 `depth` 初值给了 1，但循环又从 `{` 本身开始扫了一遍 ⇒ depth 变成 2，
结尾的 `}` 永远回不到 0 ⇒ **任何响应都会被判成「不是合法 JSON」**。
这个 bug 在 CI 编译之前发现不了，而 Python 复刻等价实现时当场暴露。

### 两条造陷阱的教训（本轮现踩）

1. **陷阱数据必须先验证「坏实现确实会挂」**。第一版陷阱 4 写的是「值里出现 `devices` 字样」，
   但 JSON 字符串值里的字面引号必须前置反斜杠 ⇒ `"devices"` 这个子串**在字符串值里根本不可能出现**
   ⇒ 连最笨的子串搜索都匹配不到 ⇒ 判据白过。改成「设备对象内嵌子对象同名字段」才真正有区分度。
2. **陷阱要带不平衡括号。`a}b{c` 左右抵消，不带字符串感知的实现也能误打误撞配对成功**（第二次栽）。

**版本号**：Windows V4.3.31 → **V4.3.32**。

## Windows V4.3.33 - 2026-09-27 - 服务日志自动轮转 + 客户端一键查看（DSH-126 / DSH-127）

**影响端**：Windows 电脑端（`windows-native/`）+ 在线相册服务（`scripts/online_gallery_service.py`）

### DSH-126 — 服务日志无上限增长（服务端，治本）

| 项 | 内容 |
|---|---|
| 现象 | `scripts/online_gallery_service.log` 是 stdout/stderr 重定向出来的，**纯追加、没有任何上限** |
| 后果 | 服务是 7x24 常驻进程，跑上几周就是几百 MB 甚至 GB 级；而这个文件就在 scripts 目录里，磁盘被自己吃光也没人发现（实测 2026-09-27 已 1MB） |
| 为什么之前没人管 | 故障表现是「磁盘慢慢变少」，没有报错、没有崩溃，谁也不会往服务日志上想 |

改动：

- 新增 `rotate_log_if_needed(path, max_bytes, backups)`：超过阈值就走一遍改名链条
  `x.log -> x.log.1 -> x.log.2 -> x.log.3`，最老的踢掉。
- 新增 `RotatingLogStream`：接管 `sys.stdout` / `sys.stderr`，写满阈值自动切。
  **写和切共用一把锁** —— 服务是 `ThreadingHTTPServer`，print() 会同时从好几个线程进来，
  而轮转要关掉旧文件再开新的，撞上就会往已关闭的句柄里写。
- 阈值：`LOG_MAX_BYTES = 8MB`、`LOG_BACKUP_COUNT = 3`、`LOG_CHECK_EVERY_CHARS = 200000`
  （每累计 20 万个字符才 `stat` 一次磁盘，不能每条日志都碰一次）。
- **启动时先切一次**：上次跑大的文件别一直顶着等写满 20 万字符。

### DSH-126-B — 两个都是「静默失效」，靠变异验证才逼出来

| # | 错法 | 后果 | 修法 |
|---|---|---|---|
| 1 | 在文件**还开着**的时候 `os.replace` | Windows 上直接 PermissionError，而本类异常一律吞掉 ⇒ **日志永远轮转不了且零报错** | 顺序必须是「先关 -> 再切 -> 再开」（跟 Linux 上「先改名再关」的直觉相反） |
| 2 | 轮转后 `open()` 失败没兜底 | 磁盘满 / 目录被删 / 权限变了 ⇒ 异常冒出 `print()` ⇒ `ThreadingHTTPServer` 的请求线程直接挂 | 降级到 `os.devnull`。日志没了能忍，服务挂了不行 |

这两条**都不是靠读代码看出来的**，是跑变异验证时测试没过才暴露的 —— 第一版实现
在真机上「看起来在轮转，磁盘上文件却一直在涨」。

### DSH-127 — 客户端一键查看服务日志

设置窗口「在线相册服务」里加了「日志」按钮，右边还显示日志当前多大：

- 点「日志」用系统默认的程序打开（`ShellExecuteW` + `open`，不写死 notepad ——
  用户可能装了别的编辑器）。
- 大小那一行跟服务状态一起每 5 秒刷新；服务从没跑过时显示「日志 暂无」。
- 日志路径优先取「已配置的脚本路径」的父目录，没配置过再按 exe 位置逐级向上找。

### 新增判据与验证

`scripts/test_online_gallery_service.py` 新增 `TestLogRotation`（10 条），
服务端套件 **29 -> 39 项全绿**。

| # | 判据 |
|---|---|
| 01-03 | 改名链条 / 没超阈值一刀别切 / 文件不存在时是 no-op |
| 04-05 | 备份链顺序（`.1` 最新） / 备份份数封顶 3 份，最老的被踢 |
| **06** | ★核心★ **通过流写入必须真的会轮转** —— 专抓「文件开着改不了名」的静默失效 |
| 07-08 | 没到阈值不能切 / 切完之后必须继续往新文件里写 |
| **09** | 重开失败必须降级，不能把调用方炸掉 |
| 10 | 默认阈值量级合理（1MB ≤ 单文件上限 ≤ 64MB） |

**闸门有效性（不是绿色就算数）**：`_verify_dsh126.py` 造了 5 个错误实现，
逐个确认测试会 FAIL 再逐字节还原：

| 变异 | 期望 | 实际 |
|---|---|---|
| M1 阈值判定反转（`<` 改 `>`） | FAIL | FAIL（5 条挂） |
| M2 `_rotate` 先 flush 不 close（回到静默失效） | FAIL | FAIL |
| M3 备份链改成正序 | FAIL | FAIL |
| M4 阈值放大 10 万倍（永远不触发） | FAIL | FAIL |
| M5 去掉「重开失败降级」 | FAIL | FAIL |

**5/5 全部被抓到**，还原后 sha256 与原始一致、回归回绿。

**版本号**：Windows V4.3.32 → **V4.3.33**。

## 服务端 DSH-128 - 2026-09-27 - 「作品移走登记表」只增不减，常驻内存单调增长

**影响端**：仅在线相册服务（`scripts/online_gallery_service.py`）。
客户端（Windows / Android / iOS）无改动，**Windows 版本号保持 V4.3.33**。

### 问题

`WorkScanner._moved_works` 是「作品被移走之后还能按 id 找回原图」的登记表。
它在 `__init__` 里初始化之后**只增不减**：

| 往里塞 | 往外拿 |
|---|---|
| 扫阶段目录（`_scan_stage_folder`，每条作品都登记） | restore 回「已发送0次」时 pop 一条 |
| 按 id 定位（`resolve_stage_work`） | —— |
| 作品被移走（use-work / 移垃圾库 / restore） | —— |

服务是 7x24 常驻进程 ⇒ 这个字典单调增长，跑几个月就是上万条。

关键在**每条存的是什么**：不是路径字符串，而是**完整作品对象**，含
`copyText`（V4.5 全系 11 个版本的文案）和 `images` 列表 —— 单条几 KB 到十几 KB。
所以这不是「多占几 KB」，而是常驻内存单调涨到**上百 MB**，且零报错。

### 修法

- 新增 `WorkScanner.prune_moved_works(limit=None)`：清掉路径已不存在的条目，
  并把总量压回 `MOVED_WORKS_LIMIT = 800` 以内。
- 挂在 `scan()` **真正全盘扫描的收尾**，不挂缓存命中的提前返回路径上
  （清理要 stat 每一条，没必要在高频短路径上反复跑）。

### ⛔ 判据只能用「路径还在不在」

**绝不能**按「这次 scan 的结果里有没有」来删 —— `scan()` 只扫「已发送0次」，
而 `_moved_works` 里登记的绝大多数是「已发送1次」/「垃圾作品」里的作品，
按 scan 结果删会**当场误删一大片**，把「移走后取原图」的兜底直接干掉。
路径都没了的条目本来就救不了任何请求（`get_work()` / `image_name_index()`
里也都有 `os.path.exists` 校验），留着只是白占内存。

### 判据与验证

`scripts/test_online_gallery_service.py` 新增 `TestMovedWorksPruning`（7 条），
服务端套件 **39 -> 46 项全绿**。

| # | 判据 |
|---|---|
| 01-03 | 陈旧条目清掉 / **路径还在的一条都不能删** / 混合场景只清陈旧的 |
| 04 | 超过上限要压回来（limit 做成可注入参数，测试不必造上千个目录） |
| 05 | 真的扫一轮之后会被清掉（证明清理挂在了扫描路径上） |
| 06 | 幂等：连清两次结果不变 |
| 07 | 默认上限量级合理（100 ≤ limit ≤ 5000） |

`_verify_dsh128.py` 造 4 个错误实现，逐个确认会 FAIL 再逐字节还原：

| 变异 | 期望 | 实际 |
|---|---|---|
| M1 `prune` 直接 `return 0`（等于没清理） | FAIL | FAIL（5 条挂） |
| M2 判据反转（删活着的、留陈旧的） | FAIL | FAIL（6 条挂） |
| M3 去掉上限压制 | FAIL | FAIL |
| M4 去掉 `scan` 里的清理调用 | FAIL | FAIL |

**4/4 全部被抓到**，每轮 sha256 与原始一致，还原后回归回绿。

### 事故：变异脚本被 SIGTERM 打断，把源文件留在了变异态

第一轮跑 `_verify_dsh128.py` 时前台 120 秒超时被 `SIGTERM` 杀掉。
**`try/finally` 里的还原没有执行** —— SIGTERM 默认直接终止进程，不抛异常。
结果 `if extra > 0:` 被换成 `if False:` 留在了源文件里（size 少了 4 字节），
得专门写 `_restore_dsh128.py` 反向替换才救回来（不能用 `git checkout`，
因为 DSH-128 的改动还没 commit，那样会把改动一起丢掉）。

已加固：变异脚本注册 `SIGTERM` / `SIGINT` 处理器，收到信号先还原再退出；
并且这类脚本一律**后台运行**（跑 6 轮测试约 3.5 分钟，远超前台超时）。

## 服务端 DSH-129 - 2026-09-27 - 缩略图「单飞登记表」只增不减，常驻进程慢泄漏
...
缩略图生成是 CPU 密集活，所以 `build_thumbnail_bytes()` 用「每个缓存键一把锁」
做**单飞**（同一张图并发生成时只让一个线程真正跑 PIL，其余等结果）。

但登记表 `THUMB_INFLIGHT` 从模块加载之后就：

| 往里塞 | 往外拿 |
|---|---|
| `_inflight_lock()` 每次调用都 `THUMB_INFLIGHT[key] = lock` | **全文件没有任何一行** |

键是 `md5(路径_mtime)` ⇒ 每请求过一张新图、或某张图的 mtime 变了，就**永久多一条**。
服务 7x24 常驻，跑几个月堆出几万个锁对象，单调增长、**零报错零日志**。

同类问题还有一张 `_phone_sync_last`（手机「顺手回读」的冷却表，按 IP 记录）：
手机 DHCP 每换一个 IP 就永久留一条。单条只有几十字节，但同属「没人负责往外拿」。

> 这两条都是同一条排查手法挖出来的：**凡是在常驻进程里「往容器里加东西」的代码，
> 都去问一句「谁负责往外拿」**。答不上来就是慢变量隐患。

### 修法

- 登记表的值从「裸锁」改成「带引用计数的票」：`{"lock": ..., "users": N}`。
  `_inflight_acquire()` 时 +1，`_inflight_release()` 时 -1，**归零才真正删除**。
- `build_thumbnail_bytes()` 的调用点用 `try/finally` 包住 —— 失败路径
  （图打不开 / PIL 抛异常）也必须 release。
- 新增 `prune_inflight_entries(limit)` 作**兜底安全网**（正常路径走不到），
  `MAX_INFLIGHT_ENTRIES = 2000`。
- 手机冷却表抽出 `_prune_phone_sync_cooldown(now, cooldown)`，挂在心跳路径上。

### ⛔ 最关键的一条：不能按 LRU / 上限直接删

删掉一把**还有线程正在等**的锁，单飞保护会**静默失效** —— 不报错、不崩溃，
只是同一张图被重复生成，CPU 白白翻倍。这种 BUG 几个月都查不出来。

所以：

- `prune_inflight_entries()` **绝不动 `users > 0` 的条目** —— 宁可超限也不破坏单飞；
- 它只是兜底，真正的回收靠引用计数归零。

### 健康可见性

`thumbnail_health()`（/api/online/status）新增三个字段：

| 字段 | 含义 |
|---|---|
| `inflightEntries` | 登记表当前条目数（**正常应该在请求结束后回落到 0**） |
| `inflightInUse` | 当前有多少线程正引用着 |
| `inflightLimit` | 安全网上限 |
...
`scripts/test_online_gallery_service.py` 新增 `TestThumbInflight`（11 条），
服务端套件 **46 -> 57 项全绿**；三端对等闸门 **49/49**。

| # | 判据 |
|---|---|
| 01-03 | release 后条目真删 / 还在用时留着 / 两个调用方拿到**同一把锁**且只占一条 |
| 04-05 | 安全网只挑没人引用的下手；全都在用时**一条都不删** |
| 06-07 | 生成失败 / 生成成功，两条路径都必须把登记表清干净 |
| 08 | 健康快照必须能看见登记表大小 |
| 09-10 | 默认上限量级合理；源码级回归闸门（裸锁写法不能回来） |
| 11 | 手机冷却表：陈旧 IP 清掉、只过 1.5 倍冷却期的**必须留下** |

`_verify_dsh129.py` 造 7 个错误实现，逐个确认会 FAIL 再逐字节还原：

| 变异 | 期望 | 实际 |
|---|---|---|
| M1 release 不再真删（`pop` → `get`，回到只增不减） | FAIL | FAIL（4 条挂） |
| M2 + M2b 去掉 `finally`（release 只走成功路径） | FAIL | FAIL（见下） |
| M3 安全网允许删在用的条目 | FAIL | FAIL |
| M4 手机冷却表阈值从 2 倍降回 1 倍 | FAIL | FAIL |
| M5 健康快照不再暴露登记表 | FAIL | FAIL |
| M6 每次 acquire 都新建一把锁（单飞失效） | FAIL | FAIL |

**7/7 全部被抓到**，每轮 sha256 与原始一致，还原后回归回绿。

### 补刀：被 SyntaxError 抓到的变异不算数

M2（把 `try:` 改成 `if True:`）单独施加时，原来的 `finally:` 没了配套的 try，
解释器**连文件都 import 不进来**，根本没跑到任何断言 —— 那只能证明
「语法检测器在工作」，证明不了判据有效。

于是 `_verify_dsh129b.py` 把 M2 + M2b **成对施加**，让代码保持语法合法
（`if True: ... elif False:`），先断言 `import ok`，再确认是**行为级** FAIL：
`test_06`（失败路径）+ `test_07`（成功路径）+ `test_10`（源码级）挂掉。
这才算真的证明了「release 必须挂 finally」这条判据会响。

> 顺带记两个抽失败用例的正则坑：①写成 `\.(test_\d+)\)` 一个都匹配不上
> （编号后面还有函数名），会得出「exit=1 但没有失败用例」的自相矛盾结论；
> ②放宽成 `TestThumbInflight\.(test_\d+)_` 又把 `-v` 的 `... ok` 通过行算进去，
> 打印出「11 条全挂」这种一看就假的结论。**只认 `FAIL:` / `ERROR:` 开头的汇总行。**

### 顺带发现：测试里的 1×1 PNG 是坏图

`test_online_gallery_service.py` 文件头的 `PNG_1PX` 常量（给扫描器当占位图用的）
