...
## Android 0.6.23 / iPhone 0.6.10 平台按钮布局 - 2026-08-17

- 作品卡片上的小红书、抖音按钮改为上下两行，各自占满卡片宽度，避免窄屏并列挤压文字。
- 两个平台仍保持独立计数、同一作品清理计时和原有分享链路，只有布局改变。

## Android 0.6.22 / iPhone 0.6.9 平台文案分发 - 2026-08-16

- 作品卡片的“复制并分享”改为“小红书 / 抖音”两个平台按钮；每个平台独立显示点击次数，但两个按钮共用同一个作品 used 状态和首次使用清理计时。
- TXT 支持 `<<<COPY_FORMAT:2>>>`、`XHS`、`DOUYIN` 机器标记，复制时只取对应段并保留内部换行；旧 TXT 无标记时两个平台都继续使用全文。
- 缺少目标段落或文案损坏时给出可行动错误，不增加次数、不删除图片；图片仍复用原有系统分享桥接。
- Android `meta.properties` 与 iOS `_相册状态.json` 新增平台计数、used、首次使用和删除计划字段；本地文件清理后把必要计数写入历史，按相同 workId 重推不会把计数清零。
- 新增 Android/iOS 解析器回归测试，覆盖双段、旧格式、缺段和损坏 TXT。
