...
## Android 0.6.11 / iPhone 0.6.7 / Windows V4.1.2 - 2026-08-01
...
- 修复 MIUI 拒绝向非导出接收器投递系统 `DOWNLOAD_COMPLETE` 广播的问题；接收器开放给系统投递后仍只处理已登记下载 ID，并执行完整安装包校验。
- 0.6.10 作为修复后的实体升级起点，最终以 0.6.10 → 0.6.11 的公网更新完成确认式流程验收。
