...
- Android“相册”0.3.10：同一 Wi-Fi 接收、作品识别、数量徽标、复制文案、系统分享、次数记录和回收站；
- iPhone“相册”0.1.3：读取用户授权的作品目录、递归识别作品、数量徽标、复制文案、系统分享、次数记录和回收站；
...
2. “相册”首次选择一次该总文件夹。0.1.3 会按“总文件夹 → 作品包 → 单个作品”有限递归寻找图片与 TXT，并忽略 `.Trash` 等隐藏目录。
...
- iPhone 0.1.3 递归扫描、隐藏目录提示和数量徽标需要重新安装到实体机验收；
