...
- Android“相册”0.3.9：同一 Wi-Fi 接收、作品识别、复制文案、系统分享、次数记录和回收站；
- iPhone“相册”0.1.2：读取用户授权的作品目录、递归识别作品、复制文案、系统分享、次数记录和回收站；
...
2. “相册”首次选择一次该总文件夹。0.1.2 会有限递归寻找作品并忽略 `.Trash` 等隐藏目录。
...
| iOS 源码 | `tools/device-share-hub/ios/Album` | SwiftUI 客户端 |
...
10. 未经过实体手机实际操作的能力必须标记“已实现未验收”。

## 已知待办

- iPhone 0.1.2 递归扫描和隐藏目录提示需要重新安装到实体机验收；
- iOS 快捷指令正式 iCloud 链接尚未制作；
- iOS 跨自然日回收和 7 天清理尚未完成实体机跨日验收；
- Android `OnlineService` 的发现信息仍有历史硬编码版本字段，下一次 Android 正式发版应改为读取包版本并同步升级版本号；
