...
当前源码版本：`0.5.3` build 22，同一 IPA 兼容 iOS 12+；应用前台支持最新剪切同步和一小时局域网中转队列。第二备用 GitHub Actions run `30613740118` 的模拟器测试、真机 SDK 构建和 IPA 结构校验通过；数据线连接设备的应用清单已确认侧载版本 0.5.3/build 22、最低 iOS 12.0、描述文件验证成功。该记录证明覆盖安装，不代替前台业务功能逐项操作。
