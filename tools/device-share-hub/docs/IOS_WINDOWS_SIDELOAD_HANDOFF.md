...
当前开发版本：`0.1.3`（递归扫描、`.Trash` 过滤与作品数量徽标，实体机验收前仍为“已实现未验收”）
...
验收边界：以上只证明“构建产物可在实体 iPhone 上签名并安装”。应用启动、文件夹授权、作品扫描、文案复制、多图分享、分享次数、次日回收和 7 天清理仍为“已实现未验收”。
