...
当前源码版本：`0.6.8` build 27，同一 IPA 兼容 iOS 12+；本轮把设置、回收站等二级页面的返回按钮统一为中文“返回”，不改变应用标识与已有数据。正式 IPA 需以本轮 GitHub Actions 的真机 SDK 构建和包内版本校验为准；当前实体 iPhone 仍是 0.6.7/build 26，完成覆盖安装前不得写成已安装。
