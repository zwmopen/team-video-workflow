...
## DSH-069 作品卡操作需要统一为整卡面板（源码候选）

- 现象：Android/iPhone 作品卡操作纵向排列，手机屏幕上需要反复滚动；删除作品还需要进入其他路径。
- 修复：两端改为整卡布局，统一“预览 / 发抖音 / 发小红书 / 删除”；删除先确认，再移入“相册回收站”；已分享平台即时灰化但不禁用。
- 当前证据：源码静态检查待完成；本轮云端构建、更新索引、安装和真实手机 UI 操作尚未完成，不能提前宣称已发布。
- 回归要求：窄屏按钮可读且不截断；取消删除不改变文件；确认删除后能在回收站恢复；分享失败时灰态恢复到持久化计数；旧版传输和远程 ACK 不受影响。
