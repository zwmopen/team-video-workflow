...
- 修复：只有设备名或型号能确认一致时才合并；匿名接口始终显示独立“安卓手机”卡片，并提示切换为文件传输。
- 证据：K60 仅开放调试接口、红米 9A 同时通过 Wi‑Fi 在线时复现了误合并；修复后的自动构建与同场景界面复核属于本版本发布门槛。
- 回归要求：多台安卓在线时不得按数量猜身份；无法确认身份时宁可分开显示，也不能把发送通道关联到错误设备。
