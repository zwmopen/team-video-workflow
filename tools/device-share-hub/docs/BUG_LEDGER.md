...
- 根因：部分路由器会抑制 Wi‑Fi 广播回复；手机接收端和 `/v2/info` 仍可访问，但 Windows 没有共享技能已有的私有 `/24` 主动探测兜底。
- 修复：保留 UDP 即时发现，同时新增默认网关所在私有 `/24` 的原生 WinHTTP 并发探测；只接受 protocol 2 的 `/v2/info`，每 15 秒刷新并把 35 秒内确认的设备同步到“发送到”菜单。没有依赖 Python、ADB、手工 IP 或管理员权限。
- 证据：修复前共享技能真实发现在线手机而 Windows 日志为 `send_to_synced online=0`；修复后的 MSVC、菜单和实体传送证据待下一轮候选构建后补写。
