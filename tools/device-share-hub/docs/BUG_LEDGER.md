...
- 数据边界：不按名称猜测，不改用户原图片和文案；覆盖升级前先备份应用私有数据。新的一小时清理只使用升级后首次点击生成的毫秒时间戳，旧记录不会突然批量删除。
...
- 回归：取消系统分享面板仍只记一次；旋转/恢复 Activity 不得二次记账；跨通道同内容但不同编号只保留回收状态；相同图片但文案不同不能误合并；升级前旧记录不能被一小时策略追溯删除。
