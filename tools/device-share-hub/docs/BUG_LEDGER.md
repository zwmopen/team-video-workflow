...
- 修复：打开应用仍自动检查，但发现新版先由用户确认“下载更新”；校验完成后在首页弹出版本、APK 文件名和大小，并由用户点击“安装”。应用不在首页时保留常驻通知，下次回到首页再次提示；不再强行自动启动安装器。下载完成接收器允许系统进程投递，业务层仍只接受当前已登记的下载 ID，并继续执行 SHA-256、包名、版本和签名验证。
- 证据：自动测试、Release 构建与 Release Lint；正式发布后使用仍为 0.6.8 的 Redmi Note 8 从公网完成“发现新版 → 确认下载 → 校验后安装提醒 → 点击进入系统安装器”实体回归。
- 回归要求：拒绝下载不得创建任务；下载完成不得直接抢占当前界面；安装按钮和通知都必须指向已验证的私有 APK；后台文件传送不得因更新流程修改而停止。
