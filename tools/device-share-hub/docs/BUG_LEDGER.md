...
## DSH-122-B — CI 编译反馈：宏断言里不能出现转义引号，std::max 两侧必须同类型（2026-09-27）

| 项 | 内容 |
|---|---|
| 现象 | DSH-122/123/125 首次推送后，CI run 36268800531 的 `windows-portable` 挂了，4 个编译错误 |
| 前提 | 本机没有 MSVC / g++ / clang++，**CI 是唯一的真编译闸门**。这类错误本机任何静态检查都发现不了，属于必然要交一次的学费 —— 但踩一次就要变成规则 |

### 错误 1：`std::max` 模板推导没有公共类型（3 处）

| 文件:行 | 原写法 | 报错 |
|---|---|---|
| main.cpp:4657 | `std::max(1, (client.bottom - client.top) - kSettingsScrollStep)` | C2672 + C2737 |
| main.cpp:4757 | `std::max(0, (workWidth - width) / 2)` | C2672 |
| main.cpp:4758 | `std::max(0, (workHeight - height) / 2)` | C2672 |

| 项 | 内容 |
|---|---|
| 根因 | `RECT` 的成员是 **LONG**，`LONG - int` 提升成 **long**，与字面量 `1` / `0`（int）不是同一类型 ⇒ `std::max<T>` 推导不出公共 T |
| 修法 | 显式 `static_cast<int>(...)` 统一，或字面量改用 `0L` |
| 教训 | **Win32 的 `RECT` / `POINT` 成员是 LONG 不是 int**，跟 `constexpr int` 混算时所有 `std::max` / `std::min` / `std::clamp` 都要显式统一类型。写的时候看着像 int，编译期不是 |

### 错误 2：宏断言的表达式里出现了转义引号（1 处，报了 8 个错）

| 文件:行 | 原写法 | 报错 |
|---|---|---|
| phone_panel_tests.cpp:348 | `CHECK_MSG(panel.devices[0].name == L"a\"b", "...")` | C2017 / C3688 / C2146 / C2660 / C2065 / C2143 / C2059 |

| 项 | 内容 |
|---|---|
| 根因 | `CHECK_MSG` 用 `#expression` 做字符串化，而 **MSVC 传统预处理器的字符串化只保证给 `"` 前面补反斜杠，不保证给已有转义序列的反斜杠再补一层** ⇒ `\"` 被展开成提前闭合的坏字面量，后面的 `b` 变成裸标识符，一串级联报错 |
| 为什么本机发现不了 | 我的结构检查器（`_lint_cpp_structure.py`）按 **C++ 语义** 处理转义 —— 它看到的是合法的 `\"`；问题出在**预处理器**这一层，不在语法层 |
| 修法 | 期望值一律先算进变量再断言：`const std::wstring expected = std::wstring(L"a") + wchar_t('"') + L"b";` |
| ⚠️ 边界 | **`\\`（转义反斜杠）是安全的，不用改**。`desktop_shortcut_tests.cpp` 里有一堆 `L"\\online_gallery.cmd"` 在 CI 上编译通过 —— 因为字符串化后的文本**只用于打印错误信息**，内容差一个反斜杠不影响编译，只要能解析就行。真正会炸的只有转义引号 `\"` |
| 连带 | 把 `TestBuildSyncRequestBody` 里所有含引号/反斜杠的期望值也一并改成先算进变量，消除同类隐患 |
| 教训 | **新规则（已生效）：宏断言的表达式里不许出现转义引号。期望值一律走变量。** 用 `#` 字符串化的宏都有这个坑，不只是 CHECK_MSG |

### 结果

修复后第二轮推送（`4962451`），见 CI 运行结果。

## DSH-126 — 在线相册服务日志无上限增长（2026-09-27）

| 项 | 内容 |
|---|---|
| 现象 | `scripts/online_gallery_service.log` 一直变大，2026-09-27 实测已 1MB，且**没有任何回收机制** |
| 根因 | `_setup_streams()` 里 `open(os.path.join(log_dir, log_name), "a", ...)` 把 `sys.stdout` / `sys.stderr` 重定向到文件后，**从头到尾只 open 一次、只 append**，既没有 `RotatingFileHandler` 也没有任何大小判断 |
| 后果 | 服务是 7x24 常驻进程，跑几周就是 GB 级。文件就在 scripts 目录里，磁盘被自己吃光也不会有人发现 —— 没有报错、没有崩溃 |
| 为什么之前没暴露 | 故障表现是「磁盘慢慢变少」，属于**慢变量**，没人会往服务日志上想 |
| 修法 | 新增 `rotate_log_if_needed()`（改名链条 `x.log -> .1 -> .2 -> .3`）+ `RotatingLogStream`（接管 stdout/stderr，写满自动切）。阈值 8MB / 留 3 份 / 每 20 万字符 stat 一次；**启动时先切一次** |
| 为什么不用 `logging.handlers.RotatingFileHandler` | 这里接管的是**整个进程**的 sys.stdout / sys.stderr（服务日志基本都是 `print()` 打出来的），Handler 那一套管不到 |
| 线程安全 | 服务是 `ThreadingHTTPServer`，print() 会同时从多个线程进来；轮转要「关旧 + 开新」，跟并发写撞上就会往已关闭的句柄里写。所以写和切共用一把 `threading.Lock` |
| 类型 | **隐患（慢变量）** |

### DSH-126-B — 两个静默失效，靠变异验证才逼出来

| # | 错法 | 后果 | 修法 |
|---|---|---|---|
| 1 | 在文件**还开着**的时候 `os.replace(path, path + ".1")` | Windows 上直接 `PermissionError`；而本类所有异常都是吞掉的（`except Exception: return False`）⇒ **日志永远轮转不了，且零报错** | 顺序必须是「先关 -> 再切 -> 再开」，跟 Linux 上「先改名再关」的直觉恰好相反 |
| 2 | 轮转后 `open()` 失败没有兜底 | 磁盘满 / 日志目录被手工删掉 / 权限变了 ⇒ 异常从 `_rotate()` 冒出 `print()` ⇒ **ThreadingHTTPServer 的请求线程直接挂掉** | 降级到 `os.devnull`：日志没了能忍，服务挂了不行 |

**这两条都不是读代码看出来的。** 第一版实现在真机上「看起来在轮转、异常也被吞了」，
实际 `os.replace` 每一步都失败 —— 是跑变异验证时 `test_06` 不过才逼出来的。

### 判据与闸门有效性

`scripts/test_online_gallery_service.py` 新增 `TestLogRotation`（10 条），服务端套件 29 -> 39 项全绿。

关键设计：**判「文件系统事实」，不判「有没有调用某个函数」**。
`test_06` 必须走 `RotatingLogStream` 而不是只测 `rotate_log_if_needed` ——
后者在文件关闭状态下是能正常工作的，完全看不出「文件开着改不了名」这个坑。

`_verify_dsh126.py` 造 5 个错误实现逐个确认会 FAIL：

| 变异 | 结果 |
|---|---|
| M1 阈值判定反转（`<` 改 `>`） | FAIL（5 条挂） |
| M2 `_rotate` 先 flush 不 close | FAIL |
| M3 备份链改成正序 | FAIL |
| M4 阈值放大 10 万倍 | FAIL |
| M5 去掉「重开失败降级」 | FAIL |

**5/5 全部被抓到**，每轮结束 sha256 与原始一致，还原后回归回绿。

## DSH-127 — 服务日志只能自己摸到 scripts 目录里去翻（2026-09-27）

| 项 | 内容 |
|---|---|
| 现象 | 服务端 DSH-126 加了自动轮转之后，用户仍然**看不到日志现在多大**，也没有一键打开的入口 |
| 影响 | 排查「手机连不上 / 扫描跳过 N 个目录」这类问题时，得自己开资源管理器摸到 scripts 目录 |
| 修法 | 设置窗口「在线相册服务」里加「日志」按钮 + 右边一行显示日志大小（跟状态一起每 5 秒刷新） |
| 细节 | ①用 `ShellExecuteW` + `open` 交给系统默认程序，**不写死 notepad**（用户可能装了别的编辑器）；②日志路径优先取「已配置的脚本路径」的父目录，用户把脚本挪走时看到的才是他真正在用的那份；③文件不存在时提示「服务至少启动过一次才会有日志」，不说「出错了」 |
| 边界 | **只查看、不删除**。清理日志是用户自己的决定，程序替他删属于越界 |
| 类型 | **易用性缺口** |

## DSH-128 — 「作品移走登记表」只增不减 ⇒ 常驻内存单调增长（2026-09-27）

| 项 | 内容 |
|---|---|
| 现象 | 无。这正是问题所在 —— 它不会报错、不会崩溃，只是**内存慢慢涨** |
| 根因 | `WorkScanner._moved_works` 在 `__init__` 初始化之后只增不减：扫阶段目录（`_scan_stage_folder`，每条作品都登记）、按 id 定位（`resolve_stage_work`）、作品被移走（use-work / 移垃圾库 / restore）都在往里塞；**只有 restore 回「已发送0次」时 pop 一条** |
| 为什么严重 | 每条存的不是路径字符串，而是**完整作品对象**，含 `copyText`（V4.5 全系 11 个版本文案）与 `images` 列表 ⇒ 单条几 KB 到十几 KB。服务 7x24 常驻，跑几个月上完万条 ⇒ 常驻内存涨到上百 MB |
| 为什么之前没人发现 | 涨得慢、无报错。唯一症状是「服务进程的内存占用好像变大了」，没人会往这里想 |
| 修法 | 新增 `prune_moved_works(limit=None)`：清掉路径已不存在的条目 + 压回 `MOVED_WORKS_LIMIT = 800`；挂在 `scan()` 真正全盘扫描的收尾 |
| ⛔ 最容易犯的错 | 按「这次 scan 的结果里有没有」来判断该不该删 —— `scan()` **只扫「已发送0次」**，而登记表里绝大多数是「已发送1次」/「垃圾作品」的作品 ⇒ 会当场误删一大片，把「移走后取原图」的兜底干掉。**判据只能用「路径还在不在」**（路径没了的条目本来就救不了任何请求，`get_work()` / `image_name_index()` 里也都有 `os.path.exists` 校验） |
| 类型 | **慢变量隐患**（与 DSH-126 的日志无限增长同类） |

### 判据与闸门有效性

`TestMovedWorksPruning`（7 条），服务端套件 39 -> 46 项全绿。
`_verify_dsh128.py` 造 4 个错误实现逐个确认 FAIL：**4/4 全部被抓到**。

### 事故：变异脚本被 SIGTERM 打断，源文件被留在变异态

| 项 | 内容 |
|---|---|
| 现象 | 第一轮跑 `_verify_dsh128.py` 时前台 120 秒超时被 `SIGTERM` 杀掉，脚本正处在「已写入 M3 变异、还没还原」的窗口 |
| 根因 | **`try/finally` 里的还原不会执行** —— `SIGTERM` 默认直接终止进程，**不抛异常**，所以 `finally` 没机会跑 |
| 后果 | 源文件被留在变异态：`if extra > 0:` 被换成 `if False:`（size 少了 4 字节）。下一轮基线直接 FAIL，差点被误读成「实现有问题」 |
| 救援 | 写 `_restore_dsh128.py` 做**带上下文的**反向替换。⛔ **不能用 `git checkout`** —— DSH-128 的改动还没 commit，那样会把改动一起丢掉 |
| 已加固 | ①变异脚本注册 `SIGTERM` / `SIGINT` 处理器，收到信号先还原再退出；②这类脚本（跑 6 轮测试约 3.5 分钟）一律**后台运行**，别用前台超时 |
| 教训 | **任何「改文件 → 跑测试 → 还原」三段式的脚本，都要假设它可能在中间被杀。** 光靠 `try/finally` 挡不住 `SIGTERM`；要加信号处理器，并且优先后台运行 |

## DSH-129 — 缩略图「单飞登记表」只增不减 ⇒ 常驻进程慢泄漏（2026-09-27）

| 项 | 内容 |
|---|---|
| 现象 | 无。这正是问题所在 —— 不报错、不崩溃，只是**内存慢慢涨、锁对象慢慢堆** |
| 根因 | `THUMB_INFLIGHT` 只有 `_inflight_lock()` 往里塞（`THUMB_INFLIGHT[key] = lock`），**全文件没有任何一行往外拿**。键是 `md5(路径_mtime)`，每请求过一张新图 / mtime 变了就永久多一条 |
| 为什么之前没人发现 | ①涨得慢；②零报错零日志；③**没有任何出口暴露它** —— `/api/online/status` 里看不到这个数字 |
| 修法 | 登记表的值从「裸锁」改成「带引用计数的票」：`_inflight_acquire()` +1 / `_inflight_release()` -1，归零才删；调用点用 `try/finally` 包住；`prune_inflight_entries()` 作兜底（`MAX_INFLIGHT_ENTRIES = 2000`） |
| ⛔ 最容易犯的错 | **按 LRU / 上限直接删**。删掉一把还有线程正在等的锁，单飞保护会**静默失效** —— 不报错、不崩溃，只是同一张图被重复生成、CPU 白白翻倍。这种 BUG 几个月都查不出来。所以安全网**绝不动 `users > 0` 的条目** |
| 同类 | `_phone_sync_last`（手机回读冷却表，按 IP 记录）同样只增不减，抽 `_prune_phone_sync_cooldown()` 挂在心跳路径上。阈值特意取 **2 倍**冷却期：取 1 倍的话，手机正常间隔 5 分钟来一次心跳就可能被自己上一轮的冷却判定清掉 ⇒ 冷却失效 |
| 类型 | **慢变量隐患**（与 DSH-126 日志无限增长、DSH-128 移走登记表同类） |

### 排查手法（可复用）

**凡是在常驻进程里「往容器里加东西」的代码，都去问一句「谁负责往外拿」。**
答不上来就是慢变量隐患。三特征：①无报错无崩溃；②症状是资源慢慢变少/变大；
③根因在常驻进程。

### 判据与闸门有效性

`TestThumbInflight`（11 条），服务端套件 **46 -> 57 项全绿**；对等闸门 **49/49**。

`_verify_dsh129.py` 造 7 个错误实现逐个确认 FAIL：**7/7 全部被抓到**。
其中 M2 单独施加时是被 `SyntaxError` 抓到的（`try:` 改成 `if True:` 后
`finally:` 没有配套的 try，文件 import 不进来，压根没跑到断言）——
**那不算证据**。故补 `_verify_dsh129b.py` 把 M2 + M2b 成对施加，
保持语法合法（先断言 `import ok`），确认 `test_06`/`test_07` 是**行为级** FAIL。

### 教训：抽失败用例的两个正则坑

| # | 错法 | 后果 |
|---|---|---|
| 1 | `re.findall(r"\(TestThumbInflight\.(test_\d+)\)", ...)` | 编号后面还有函数名，一个都匹配不上 ⇒ 得出「exit=1 但没有失败用例」的自相矛盾结论 |
| 2 | 放宽成 `TestThumbInflight\.(test_\d+)_` | `-v` 模式下 `... ok` 的**通过行**也带这个 test id ⇒ 打印出「11 条全挂了」这种一看就假的结论，当时居然据此判了 ✅ |

**只认 `FAIL:` / `ERROR:` 开头的汇总行** —— 那才是 unittest 对失败的权威判定。
教训的教训：**自动化结论出现「全挂」或「全过」这种极值时，先怀疑测量方式**。

### 顺带：测试夹具里那份 1×1 PNG 是坏图
