...
## 2026-09-26 DSH-113~116：局域网更新中转（手机终于能升级）+ 三个静默故障根治

**完整说明见 [`docs/LAN_UPDATE_RELAY.md`](LAN_UPDATE_RELAY.md)** —— 原理 / 链路图 / 三个接口 /
清单改了什么没改什么 / 四条硬规则 / DSH-116 详解 / 健康自查 / 排障手册 / 验收清单，一次写全。

当前版本：**Android 0.8.61（versionCode 172）**、**iPhone 0.8.44（build 116）**；在线相册服务 **1.1.0**；提交 `3e504dd`。

| DSH | 一句话 | 闸门 |
|---|---|---|
| **113** | 建**局域网更新中转**：电脑代取 GitHub 的发布清单+安装包，再从局域网端给手机。此前手机直连 GitHub 基本不可达 ⇒ **根本看不到新版本** | A16 |
| **114** | 扫描缓存 TTL 5s **小于**单次扫描耗时 5~10s ⇒ **命中率恒为 0**，改 30s + 由 watchdog 按磁盘变更失效 | A17 |
| **115** | 出网代理写死 7897，而 Clash 端口会在 7890/7897 回跳、节点还会掉 ⇒ 整条中转**静默失效**（只回 110 字节报错体） | A18 |
| **116** | 安装包缓存只按版本落名；GitHub 会以同一 `version_name` 重发**重新构建过**的包（字节不同、体积相同）⇒ 旧字节顶着新清单的 sha256，手机校验必失败 | A19 |

- 🔴 **DSH-116 是最阴的一条**：中转侧 `downloads` 在涨、`downloadFailures=0`、零报错；
  客户端侧永远「更新包校验失败」并删包 —— **两边都不报警**。
  实测同一个 `0.8.61` 至今出现过**三套不同字节**（`45635c6a…` / `362908b0…` / `4c30c5c8…`，体积全是 883308）。
- 修法：缓存名编入 sha256 前 12 位；**命中分支也重算 sha**，不一致即 `os.remove` 重下；
  新增 `shaMismatch`（错过几次）/ `cachedSha`（现在端出去的是哪个包）两个可见指标。
- 回归：契约闸门 **43/43**；服务端单测 **29 项**全过；CI `36246000789` **6/6 全绿含 publish**。
- 📱 红米13（192.168.0.101）现场仍 0.8.58/169 —— 中转已就绪，等它下次打开相册 App 自动升；
  或把工作台（Electron）拉起来由 `maybeAutoPushAppUpdates` 主动推 APK。
...
- **为什么要再出一版**：0.8.23 把「有没有数据」的判定时机修对之后，**顺带把自愈也关掉了** ——
  `tryAutoDiscoverPc()` 只写在「完全没有数据」那一支，而 0.8.22 能自愈恰恰是靠 `hadData`
  被异步快照误判成 `false`、走错分支才顺带跑了一次。净效果是「弹窗变成静默卡死」，反而更糟。
- **真机实测复现（方法可复用，不改服务端、不影响其它在线手机）**：
  用 AFC 把 App 偏好里的 `customPcServerUrl` 改成旧网段 `http://192.168.0.107:45835` 再冷启动 →
  0.8.23：**无弹窗、显示旧列表、30 秒后缓存地址未被改写**（静默卡死）。
- **铁律（改 iOS 必须守）**：**有没有缓存数据只决定「用 toast 还是用弹窗」，
  绝不能决定「要不要去找新地址」**；凡是「有数据走 A / 无数据走 B」的降级逻辑，
  都要逐支检查自愈 / 重试 / 兜底有没有被漏写在某一支里。
- **修法**：失败即在两支都调 `tryAutoDiscoverPc()`（仍受 15 秒冷却保护，不会刷屏）。
- **验证手段（本轮新增）**：
...
- **手势精准分流**：
  - `SpringScrollView` 增加落点视图检测（`findHorizontalScrollViewAt`），手指在缩略图条内滑动时父容器绝不拦截横向事件，纯粹用于横向滚动浏览图片；
  - 手指在卡片标题、按钮或留白区域左右滑动时，触发顶部分类 Tab 快速切换，彻底解决手势冲突。
- **卡片边框双向裁剪**：
  - `MainActivity.card()` 与 `previewStrip()` 重新开启 `setClipChildren(true)` 与 `setClipToPadding(true)`；
  - 缩略图横向滑动严格限定在卡片 14dp 内边距与圆角描边内部，绝不发生视觉破框溢出。
- **首屏预加载扩展**：
  - 预加载阈值由 `index < 4` 提升至 `index < 8`，彻底消除 1080p 宽屏设备上首屏卡片第 5、6 张图片未加载导致的灰白方块。

## 2026-09-13 Android v0.8.7 全面升级：全屏分类左右滑手势、设备发现主动广播、虚假接收失败通知根除

- **版本与安装对账**：Android `0.8.7`（`versionCode: 118`），已成功推送到红米 13C 5G (`69PNFQUCT4XGKZRO`) 与红米 K60 (`e3b58850`) 实机运行。
- **全屏分类左右滑智能手势**：
  - 手指在卡片底部横向缩略图条（`HorizontalScrollView`）上滑动时，仅平滑滚动浏览缩略图，不抢夺手势；
  - 手指在卡片其余任何区域（标题、留白、按钮、甚至屏幕空白处）左右滑动时，触发顶部分类 Tab 的左右切换（左滑下一分类，右滑上一分类）；
  - 分类切换采用内存缓存快速筛选（< 20ms），且顶部 Tab 栏自动平滑平移居中当前选中的分类 Pill 按钮。
- **传送界面设备发现主动广播**：
  - 传送界面新增“🔄 刷新设备”按钮，主动在 UDP 45832 发送 `ZWMDS2_DISCOVER` 广播，解决路由器 AP 隔离或组播休眠时被动发现不灵敏的问题。
- **虚假“接收失败 Broken pipe”通知根治与历史日志清空**：
  - 根治 PC 端 Electron 定时 500ms 快速探活连接关闭引发的虚假 400 异常；
  - `HttpRequest.readLine` 对 0 字节 EOF 判定为正常关闭，`OnlineService.handleHttp` 对非传输类路径故障不再弹通知、不入操作日志；
  - 传送界面新增“清空”按钮，一键清空历史虚假错误记录。
...
- **核心变更**：
  - **Android 端操作按钮原生自适应折行**：引入原生零依赖流式折行容器 `FlowLayout` 替换原单行不可换行的 `LinearLayout`，支持动态多行排版（`horizontalSpacing = dp(8)`, `verticalSpacing = dp(8)`）。当作品拥有 3 个文案版本加 1 个删除按钮时，删除按钮自适应换行至第二行，彻底解决屏幕边界被挤出或被遮挡问题；
  - **iOS 端自适应 2 行平衡网格布局**：重构 `WorkCell` 底部按钮结构为垂直双行容器 `platformContainer`。按钮数 `<= 3` 时单行等宽排布；按钮数 `> 3`（3 平台文案 + 1 删除）时自动呈现 2×2 对称网格（每行 50% 宽度，大触控热区、字迹清晰）；
  - **双端规范统一**：保持文案按钮与删除按钮操作流转一致，大屏与小屏均实现自适应。
