...
- 当前源码版本：Android versionCode 97 / versionName 0.6.59；iPhone 0.6.42/build 61。
- 两端作品卡统一为整卡布局，提供“预览 / 发抖音 / 发小红书 / 删除”；删除二次确认后移动整个作品到“相册回收站”，不直接永久删除。
- 已分享的平台按钮即时显示灰色但不禁用；平台计数、自动清理、局域网/P2P/HTTPS 中继和 ACK 逻辑未改。
- 当前只完成源码候选，尚无本轮云端构建、Release/latest.json 或真实手机安装证据；发布前必须逐项核对。
- 自检重点：Android/iOS 窄屏四按钮不截断；删除取消不动原文件；确认后作品和状态一起进回收站；回收站恢复后仍能扫描和显示历史计数。
