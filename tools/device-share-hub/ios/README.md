...
版本：0.6.45 / build 64（源码候选）

状态：源码候选；当前轮次尚未云端构建、发布或进行真机验收

0.6.45 延续同一个安装包覆盖 iOS 12 及以上，并在 `/v2/info` 上报与 Android/Windows 一致的分类库存。已移除自动截图识别、截图发送、自动读取/同步系统剪切板和相册读取权限；顶部分类统一显示“精准流量”和“泛流量”。作品卡片取消独立“预览”按钮，点击横向缩略图直接从对应图片进入全屏分页预览；底部固定横排“发抖音 / 发小红书 / 删除”三个操作，删除确认后移入回收站，平台按钮的点击灰态与计数保持不变。本轮还修复接收任务提交重试，保留同一任务 ID 并支持完成回执重放，避免网络抖动后误报“任务不存在”。普通文件、图片导入、系统分享与局域网传送不变。此版本尚未云端构建或发布。
...
`https://github.com/zwmopen/gallery-updates/releases/tag/v0.6.44-beta.1`
...
本轮 Beta IPA 的 SHA-256 为 `1db6768df098beb2c8924d5b7fb0b923d81ff53678b840bc297787fe1cd25c92`；AltStore Beta 源已更新为 iOS 0.6.31/build 50；实体检查仍需在真实 iPhone 上完成。
...
稳定源不会自动显示 Beta；添加 Beta 源后，AltStore 会发现已发布的 0.6.29/build 48。更新仍需在 AltStore 中确认一次。
