...
版本：0.6.44 / build 63（源码候选）

状态：源码候选；当前轮次尚未云端构建、发布或进行真机验收

0.6.44 延续同一个安装包覆盖 iOS 12 及以上，并在 `/v2/info` 上报与 Android/Windows 一致的分类库存。已移除自动截图识别、截图发送、自动读取/同步系统剪切板和相册读取权限；顶部分类统一显示“精准流量”和“泛流量”，作品卡片改为单列操作模块：标题缩小，顶部横向缩略图带，底部固定“预览 / 发抖音 / 发小红书”三个紧凑按钮，点击后对应平台按钮立即变灰但仍可点击，预览页可左右滑动查看下一张。本轮还修复接收任务提交重试，保留同一任务 ID 并支持完成回执重放，避免网络抖动后误报“任务不存在”。普通文件、图片导入、系统分享与局域网传送不变。此版本尚未云端构建或发布。
