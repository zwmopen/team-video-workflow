...
版本：0.8.4 / build 75（待构建发布）

状态：优化列表滑动渲染性能，引入 Image I/O 降采样与 NSCache 缩略图内存池，彻底消灭 20+ 套作品滑动卡顿
...
- 优先使用 `文案.txt`、`小红书文案.txt`、`抖音文案.txt`；排除会话追踪、生产记录、质量报告、标签、元数据和日志类 TXT，旧目录仍对其他名称 TXT 保留兼容兜底。
