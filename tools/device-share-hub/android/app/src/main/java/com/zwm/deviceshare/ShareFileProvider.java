...
public final class ShareFileProvider extends ContentProvider {
    ...
    private File resolve(Uri uri) {
        ...
        if (segments.size() != 2) throw new IllegalArgumentException("URI 格式无效");
        String taskId = segments.get(0);
        String storedName = segments.get(1);
        if (!taskId.matches("[A-Za-z0-9._-]+") || storedName.contains("/") || storedName.contains("\\")) {
            throw new SecurityException("非法文件路径");
        }
        File root = new File(getContext().getCacheDir(), "share");
        File candidate = new File(new File(root, taskId), storedName);
        try {
