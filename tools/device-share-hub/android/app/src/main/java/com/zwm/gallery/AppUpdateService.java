...
public final class AppUpdateService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        worker.execute(() -> {
            try {
                download(version.trim(), url.trim(), sha256 == null ? "" : sha256.trim());
            } catch (VerificationFailure error) {
                deletePartial(this, version);
                recordFailure(safe(error.getMessage()), true);
    ...
    private void download(String version, String url, String expectedSha256) throws Exception {
            ...
            try (InputStream input = connection.getInputStream();
                ...
                while ((count = input.read(buffer)) != -1) {
                    ...
                    if (now - lastNotify >= 500L) {
                        notifyProgress(version, downloaded, total);
                        lastNotify = now;
                    }
                }
                output.flush();
        ...
        try {
            ...
            try (InputStream input = new FileInputStream(part)) {
                actual = UpdateDownloadReceiver.sha256(input);
            }
            if (!expectedSha256.isEmpty() && !actual.equalsIgnoreCase(expectedSha256)) {
                throw new VerificationFailure("SHA-256 与发布记录不一致");
            }
            ...
            if (part.isFile()) part.delete();
            throw new VerificationFailure(safe(error.getMessage()));
        }
        ...
        SharedPreferences preferences = getSharedPreferences("device_share", MODE_PRIVATE);
        UpdateDownloadReceiver.markReady(preferences, -1L, version, fileName);
        preferences.edit()
                .remove(UpdateChecker.PREF_DOWNLOAD_RESULT)
                .remove(UpdateChecker.PREF_DOWNLOAD_ERROR)
                .apply();
        UpdateDownloadReceiver.notifyReady(this, version, fileName);
        sendBroadcast(new Intent(UpdateDownloadReceiver.ACTION_UPDATE_READY)
                .setPackage(getPackageName()));
        DiagnosticLog.write(this, "update_app_download_verified",
                version + " bytes=" + target.length());
    }

    private ConnectionResult open(String initialUrl, long existing) throws Exception {
        ...
        for (int redirect = 0; redirect <= MAX_REDIRECTS; redirect++) {
            ...
            int response = connection.getResponseCode();
            if (!isRedirect(response)) return new ConnectionResult(connection);
            String location = connection.getHeaderField("Location");
            connection.disconnect();
            if (location == null || location.trim().isEmpty()) {
                throw new IllegalStateException("下载跳转地址为空");
            }
    ...
    static boolean shouldAppend(long existing, int response, String contentRange) {
        if (existing <= 0 || response != 206 || contentRange == null) return false;
        return contentRange.toLowerCase(Locale.US).startsWith("bytes " + existing + "-");
    }

    static boolean isRedirect(int response) {
        return response == 301 || response == 302 || response == 303
    ...
    private void notifyProgress(String version, long downloaded, long total) {
        ...
        if (manager != null) manager.notify(
                NOTIFICATION_ID, progressNotification(version, downloaded, total));
    }
    ...
    private Notification progressNotification(String version, long downloaded, long total) {
        ...
        return new Notification.Builder(this, CHANNEL)
                ...
                .setContentTitle("正在下载相册 " + version)
                .setContentText(text)
    ...
    private void recordFailure(String reason, boolean verification) {
        getSharedPreferences("device_share", MODE_PRIVATE).edit()
                .putString(UpdateChecker.PREF_DOWNLOAD_RESULT, verification
                        ? UpdateChecker.RESULT_CHECKSUM_FAILED
                        : UpdateChecker.RESULT_DOWNLOAD_FAILED)
                .putString(UpdateChecker.PREF_DOWNLOAD_ERROR, reason)
                .apply();
        DiagnosticLog.write(this, verification
                ? "update_app_download_verify_failed"
                : "update_app_download_failed", reason);
    }

    private void ensureChannel() {
        NotificationManager manager = getSystemService(NotificationManager.class);
        if (manager != null) manager.createNotificationChannel(new NotificationChannel(
                CHANNEL, "更新下载进度", NotificationManager.IMPORTANCE_LOW));
    }
    ...
    private static void cleanupOldPackages(File root, File keep) {
        ...
        for (File file : files) {
            if (!file.equals(keep) && (file.getName().endsWith(".apk")
                    || file.getName().endsWith(".apk.part"))) file.delete();
        }
    }
    ...
    private static String safe(String value) {
        if (value == null || value.trim().isEmpty()) return "网络连接中断";
    ...
    private static final class ConnectionResult {
        ...
        ConnectionResult(HttpURLConnection connection) { this.connection = connection; }
    }
