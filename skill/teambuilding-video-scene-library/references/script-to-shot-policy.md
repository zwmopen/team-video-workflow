...
1. 优先读取同名 `.transcript.txt`，因为它通常包含时间戳和完整台词。
2. 其次读取同名 `.txt`，但如果它只是标题、简介或短描述，不能当成完整配镜脚本。
3. 最后读取 `.plain.txt` 作为纯文案辅助。
4. 如果 `.txt` 和 `.transcript.txt` 同时存在，不能只因为 `.txt` 存在就跳过 `.transcript.txt`。
