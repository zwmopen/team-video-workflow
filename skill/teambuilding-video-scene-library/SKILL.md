...
12. In `智能剪辑`, use the lightweight edit workbench: left audio track/library, top draggable candidate clip shelf, center 3:4 Xiaohongshu preview, right transcript/match inspector, and bottom editing timeline with a main video track plus an audio track. The MVP must support dragging clips into the main track, selecting clips, splitting timeline entries, deleting entries, opening the crop/trim editor for a selected clip, and copying the main-track clip paths.
13. `开始匹配素材` reads the selected audio transcript or ASR cache, splits transcript beats, and matches each beat to `分镜素材` using location, folder keyword, file name, and concrete activity keywords. This is the MVP review layer before true export.
